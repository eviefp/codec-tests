...
instance ( Functor m, Functor n ) => Invariant2 (Codec m n) where
    invmap2 :: (a -> c) -> (c -> a) -> (b -> d) -> (d -> b)
        -> Codec m n a b -> Codec m n c d
    invmap2 a2c c2a b2d d2b (Codec mab nba) = Codec (dimap c2a b2d mab)
        (dimap d2b a2c nba)

-- Map over 'm' and 'n'.
hoist :: (forall x. m x -> m' x) -> (forall x. n x -> n' x)
    -> Codec m n a b -> Codec m' n' a b
hoist f g (Codec mab nba) = Codec (Star $ f . runStar mab)
    (Star $ g . runStar nba)
...
manualCompose :: forall m n a b c. (forall x y. m x -> (x -> m y) -> m y)
    -> (forall x y. n x -> (x -> n y) -> n y)
    -> Codec m n b c -> Codec m n a b -> Codec m n a c
manualCompose bindM bindN (Codec mbc ncb) (Codec mab nba) = Codec (Star mac)
    (Star nca)
  where
    mac :: a -> m c
...
(***) :: forall m n a b c. Applicative m => Applicative n
    => Semigroup a => Codec m n a b -> Codec m n a c -> Codec m n a ( b, c )
(***) (Codec mab nba) (Codec mac nca) = Codec (Star mabc) (Star nbca)
  where
    mabc :: a -> m ( b, c )
...
(//) :: forall m n a b c. Alternative m => Codec m n a b -> Codec m n a c
...
-- Could this be implemented using the 'Category' instance?
-- both
...
--     -> Codec m n c e
--     -> Codec m n a (d, e)
-- both (Codec (Star a2mbc) (Star bc2na)) (Codec (Star b2md) (Star d2nb))
--     (Codec (Star c2md) (Star d2nc)) = Codec (Star a2md) (Star d2na)
--   where
--     a2md :: a -> m d
...
(^^^) :: forall m n b c d e. Applicative m => Applicative n
    => Codec m n b d -> Codec m n c e -> Codec m n ( b, c ) ( d, e )
(^^^) (Codec (Star b2md) (Star d2nb)) (Codec (Star c2me) (Star e2nc)) = Codec
    (Star bc2mde) (Star de2nbc)
  where
    bc2mde :: ( b, c ) -> m ( d, e )
...
instance ( Applicative m, Alternative n ) => Inapplicative (Codec m n x) where
    ...
    lift2 :: forall a b c. (( a, b ) -> c) -> (c -> ( a, b ))
        -> Codec m n x a -> Codec m n x b -> Codec m n x c
    lift2 ab2c c2ab (Codec (Star x2ma) (Star a2nx))
        (Codec (Star x2mb) (Star (b2nx))) = Codec (Star x2mc) (Star c2nx)
      where
        x2mc :: x -> m c
